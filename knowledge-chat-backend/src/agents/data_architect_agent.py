...
# Import SQL analyzer from utils
...
class DataArchitectAgent:
    ...
    def _get_code_enhancement_instructions(self, query: str) -> str:
        ...
        # Extract information about what needs to be enhanced
        source_column_name = ""
        new_column_name = ""
        aggregation_type = ""
        
        # Look for common patterns in the query
        add_column_match = re.search(r'add\s+(?:a\s+)?(?:new\s+)?column\s+(?:in|to|for)?\s+(?:the\s+)?(?:model\s+)?[\w\.\/]+\s+(?:named\s+)?(\w+)\s+(?:of|that\s+is)?\s+(?:the\s+)?(?:avg|average|sum|count|min|max)?\s+(?:of\s+)?(\w+)', query, re.IGNORECASE)
        
        if add_column_match:
            new_column_name = add_column_match.group(1)
            source_column_name = add_column_match.group(2)
            ...
            elif "avg" in query.lower() or "average" in query.lower():
                aggregation_type = "avg"
            else:
                aggregation_type = "avg"  # Default
        
        # Build specific instructions based on the extracted information
        specific_instructions = ""
        if source_column_name and new_column_name and aggregation_type:
            specific_instructions = f"""
# SPECIFIC ENHANCEMENT REQUEST
You need to add a new column named '{new_column_name}' that calculates the {aggregation_type} of the '{source_column_name}' column.

When implementing this:
1. Find where '{source_column_name}' is used in the model
2. Determine the best place to add the new aggregation:
   - If there's an existing aggregation CTE that already uses '{source_column_name}', add the new calculation there
   - If there's an aggregation CTE but it doesn't use '{source_column_name}', consider whether it's appropriate to add it there
   - If there's no aggregation CTE but '{source_column_name}' appears in another CTE, create a new aggregation CTE after that one
   - If it's a simple SELECT without CTEs, add the aggregation to the SELECT clause and ensure there's a GROUP BY
3. Add '{new_column_name}' to the final SELECT statement or final CTE that returns results
4. Ensure you maintain any required joins if you create a new CTE
"""
...
"""
    ...
    def _validate_code_enhancement_response(self, response: str, state: Dict[str, Any]) -> str:
        ...
        # Extract original question for context
        original_question = ""
        ...
        # Extract enhancement details (source_column, new_column, aggregation_type)
        source_column_name = ""
        new_column_name = ""
        aggregation_type = ""
        
        add_column_match = re.search(r'add\s+(?:a\s+)?(?:new\s+)?column\s+(?:in|to|for)?\s+(?:the\s+)?(?:model\s+)?[\w\.\/]+\s+(?:named\s+)?(\w+)?\s+(?:of|that\s+is)?\s+(?:the\s+)?(avg|average|sum|count|min|max)\s+(?:of\s+)?(\w+)', original_question, re.IGNORECASE)
        
        if add_column_match:
            groups = add_column_match.groups()
            if len(groups) >= 3:
                new_column_name = groups[0] or f"{groups[1]}_{groups[2]}"  # If no explicit name, use aggregation_source
                aggregation_type = groups[1]
                source_column_name = groups[2]
            else:
                # Fallback pattern for different question phrasing
                alt_match = re.search(r'add\s+(?:a\s+)?(?:new\s+)?column\s+(?:for|with)?\s+(?:the\s+)?(avg|average|sum|count|min|max)\s+(?:of\s+)?(\w+)', original_question, re.IGNORECASE)
                if alt_match:
                    aggregation_type = alt_match.group(1)
                    source_column_name = alt_match.group(2)
                    new_column_name = f"{aggregation_type}_{source_column_name}"
        
        # If we couldn't extract information but "average" and a column name are mentioned
        if not source_column_name and ("average" in original_question.lower() or "avg" in original_question.lower()):
            # Look for any mentioned column
            column_match = re.search(r'column\s+(\w+)', original_question, re.IGNORECASE)
            if column_match:
                source_column_name = column_match.group(1)
                aggregation_type = "avg"
                new_column_name = f"avg_{source_column_name}"
        
        logger.info(f"Validating code enhancement response")
        ...
        if model_content and enhanced_code:
            # Use our new SQL analyzer to verify the enhancement
            try:
                # First, extract the enhancement request details from the original question
                original_question = ""
                source_column_name = ""
                new_column_name = ""
                aggregation_type = "avg"  # Default
                ...
                # Extract column names and aggregation type from the question
                add_column_match = re.search(r'add\s+(?:a\s+)?(?:new\s+)?column\s+(?:in|to|for)?\s+(?:the\s+)?(?:model\s+)?[\w\.\/]+\s+(?:named\s+)?(\w+)\s+(?:of|that\s+is)?\s+(?:the\s+)?(?:avg|average|sum|count|min|max)?\s+(?:of\s+)?(\w+)', original_question, re.IGNORECASE)
                ...
                if add_column_match:
                    ...
                    # Determine aggregation type
                    if "sum" in original_question.lower():
                        aggregation_type = "sum"
                    elif "count" in original_question.lower():
                        aggregation_type = "count"
                    elif "min" in original_question.lower() or "minimum" in original_question.lower():
                        aggregation_type = "min"
                    elif "max" in original_question.lower() or "maximum" in original_question.lower():
                        aggregation_type = "max"
                    elif "avg" in original_question.lower() or "average" in original_question.lower():
                        aggregation_type = "avg"
                
                # Check if the LLM's enhancement properly addressed the request
                if source_column_name and new_column_name:
                    # Analyze the original and enhanced SQL
                    original_analysis = analyze_sql_model(model_content, source_column_name)
                    enhanced_analysis = analyze_sql_model(enhanced_code, source_column_name)
                    
                    # Check if the new column was actually added
                    column_was_added = False
                    
                    # Look for the new column in the enhanced code
                    new_col_pattern = fr'{new_column_name}\b|{aggregation_type}\s*\(\s*{source_column_name}\s*\)\s+as\s+\w+'
                    if re.search(new_col_pattern, enhanced_code, re.IGNORECASE):
                        column_was_added = True
                    
                    # If the column wasn't added or the enhancement is too similar to original,
                    # try to generate our own enhancement
                    import difflib
                    similarity_ratio = difflib.SequenceMatcher(None, model_content.strip(), enhanced_code.strip()).ratio()
                    
                    if not column_was_added or similarity_ratio > 0.95:
                        logger.info(f"LLM enhancement didn't add column or was too similar (similarity: {similarity_ratio}). Generating our own enhancement.")
                        
                        # Generate our own enhancement using the structured analysis
                        enhancement_result = generate_enhancement_modifications(
                            model_content,
                            original_analysis,
                            source_column_name,
                            new_column_name,
                        ...
                        if enhancement_result["success"]:
                            # Use our improved code instead
                            improved_code = enhancement_result["enhancedCode"]
                            
                            # Update the response
                            logger.info("Successfully generated improved enhancement")
                            
                            # Create a description of what we changed
                            if "details" in enhancement_result:
                                details = enhancement_result["details"]
                                description = "Implemented precise SQL changes to add the requested aggregation:\n"
                                
                                if "modifiedCte" in details:
                                    description += f"- Added {aggregation_type}({source_column_name}) as {new_column_name} to the {details['modifiedCte']} CTE\n"
                                elif "createdCte" in details:
                                    description += f"- Created new {details['createdCte']} CTE to calculate {aggregation_type}({source_column_name})\n"
                                
                                if "addedToFinal" in details and details["addedToFinal"]:
                                    description += f"- Added {new_column_name} to the final result set\n"
                                
                                description += "- Maintained the original query structure and DBT practices"
                            else:
                                description = f"Added {aggregation_type}({source_column_name}) as {new_column_name} to the query."
                            
                            # Update the original LLM response with our enhanced code
                            response = re.sub(
                                r'```sql\s*' + re.escape(enhanced_code) + r'\s*```',
                                f"```sql\n{improved_code}\n```",
                                response
                            )
                            
                            # Update or add the enhancement summary
                            summary_pattern = r'## Enhancement Summary([\s\S]*?)(?=##|$)'
                            summary_match = re.search(summary_pattern, response)
                            
                            if summary_match:
                                response = response.replace(
                                    summary_match.group(0),
                                    f"## Enhancement Summary\n{description}\n\n"
                                )
                            else:
                                # Add a summary if not present
                                response = f"## Enhancement Summary\n{description}\n\n" + response
                            
                            # Return the updated response
                            return response
                        else:
                            # If our enhancement also failed, add a note to the response
                            logger.warning(f"Automatic enhancement failed: {enhancement_result['reason']}")
                            add_note = f"\n\n**Note:** The requested enhancement to add `{aggregation_type}({source_column_name}) as {new_column_name}` "
                            add_note += f"requires careful implementation due to the structure of this model. "
                            add_note += f"Consider: {generate_suggested_approach(original_analysis, source_column_name, new_column_name, aggregation_type)}"
                            
                            return response + add_note
            except Exception as e:
                logger.error(f"Error performing enhanced validation: {str(e)}")
                # Continue with regular validation if our enhanced validation fails
            
            # Fall back to the original validation approach if needed
            import difflib
            similarity_ratio = difflib.SequenceMatcher(None, model_content.strip(), enhanced_code.strip()).ratio()
...
"""
    ...
    def _format_code_enhancement_for_prompt(self, state, model_details, formatted_results):
        ...
        # Extract enhancement details from the question to better analyze the SQL
        source_column_name = ""
        new_column_name = ""
        aggregation_type = ""
        ...
        for model in model_details:
            ...
            if model_name and content:
                ...
                # Add enhanced SQL analysis using our new analyzer
                if source_column_name:
                    try:
                        # Use our SQL analyzer to provide a detailed structure analysis
                        sql_analysis = analyze_sql_model(content, source_column_name)
                        
                        # Find the best modification target
                        target = find_best_modification_target(sql_analysis, source_column_name)
                        
                        # Add enhanced structural analysis to the prompt
                        code_section += "\n## ENHANCED SQL STRUCTURE ANALYSIS:\n"
                        
                        # Add information about CTEs
                        if sql_analysis["hasCTEs"]:
                            code_section += f"### Common Table Expressions (CTEs): {len(sql_analysis['ctes'])}\n"
                            
                            for cte in sql_analysis["ctes"]:
                                has_source = "✓" if "hasSourceColumn" in cte and cte["hasSourceColumn"] else "✗"
                                code_section += f"- `{cte['name']}` (Type: {cte['type']}) - Contains source column: {has_source}\n"
                            
                            # Add information about where the source column appears
                            if sql_analysis["sourceColumnLocations"]:
                                code_section += f"\n### Source Column '{source_column_name}' References:\n"
                                for loc in sql_analysis["sourceColumnLocations"][:5]:  # Limit to first 5 for brevity
                                    code_section += f"- Line {loc['lineNumber']} ({loc['context']}): `{loc['line']}`\n"
                            
                            # Add recommended modification approach
                            code_section += f"\n### Recommended Modification Approach:\n"
                            code_section += f"- {target['reason']}\n"
                            
                            if target["targetType"] == "aggregation_cte":
                                code_section += f"- Add {aggregation_type}({source_column_name}) as {new_column_name} to the {target['targetName']} CTE\n"
                                code_section += f"- Then add {new_column_name} to the final SELECT or final CTE\n"
                            elif target["targetType"] == "new_cte":
                                code_section += f"- Create a new CTE named {target['targetName']} after {target.get('baseCteName', 'an appropriate CTE')}\n"
                                code_section += f"- Include {aggregation_type}({source_column_name}) as {new_column_name} in this new CTE\n"
                                code_section += f"- Add proper joins to connect this new CTE to the query\n"
                            elif target["targetType"] == "select_clause":
                                code_section += f"- Add {aggregation_type}({source_column_name}) as {new_column_name} directly to the SELECT clause\n"
                                if not sql_analysis["groupBys"]:
                                    code_section += f"- Add an appropriate GROUP BY clause for non-aggregated columns\n"
                    except Exception as e:
                        logger.error(f"Error performing enhanced SQL analysis: {str(e)}")
                        # Fall back to standard analysis if our enhanced analysis fails
                
                # Add the standard model structure analysis as a fallback
                ...
                # Add specific enhancement guidance
                code_section += "\n### Enhancement Guidelines:\n"
                
                if source_column_name and new_column_name and aggregation_type:
                    code_section += f"- Add a new column called '{new_column_name}' that computes the {aggregation_type} of '{source_column_name}'\n"
                    code_section += f"- Ensure the new column is available in the final output\n"
                    code_section += f"- Add appropriate comments explaining the new calculation\n"
    ...
    def _create_lineage_visualization(self, model_details):
        ...
        def extract_and_add_columns(model_name, content, is_main_model=False):
            ...
            # For upstream models with no content or no columns, generate plausible columns
            if not columns:
                logger.info(f"No columns extracted for model {model_name}. Generating default columns.")
                
                # Convert model name to lowercase for easier pattern matching
                model_name_lower = model_name.lower()
                
                # More comprehensive pattern matching for various source and staging models
                if "stg_" in model_name_lower or "source" in model_name_lower:
                    # Order related tables
                    if any(term in model_name_lower for term in ["order", "orders"]):
                        columns = [
                            {"name": "order_key", "data_type": "integer", "expression": "", "description": "Primary order identifier"},
                            {"name": "customer_key", "data_type": "integer", "expression": "", "description": "Reference to customer"},
                            ...
                            {"name": "total_amount", "data_type": "decimal", "expression": "", "description": "Total order amount"}
                        ]
                    # Customer related tables
                    elif any(term in model_name_lower for term in ["customer", "customers", "client", "clients"]):
                        columns = [
                            {"name": "customer_key", "data_type": "integer", "expression": "", "description": "Primary customer identifier"},
                            ...
                            {"name": "address", "data_type": "string", "expression": "", "description": "Customer address"},
                            {"name": "phone", "data_type": "string", "expression": "", "description": "Customer phone number"}
                        ]
                    # Product related tables
                    elif any(term in model_name_lower for term in ["product", "products", "item", "items"]):
                        columns = [
                            {"name": "product_key", "data_type": "integer", "expression": "", "description": "Primary product identifier"},
                            {"name": "name", "data_type": "string", "expression": "", "description": "Product name"},
                            {"name": "description", "data_type": "string", "expression": "", "description": "Product description"},
                            {"name": "price", "data_type": "decimal", "expression": "", "description": "Product price"},
                            {"name": "category", "data_type": "string", "expression": "", "description": "Product category"}
                        ]
                    # Line item related tables
                    elif any(term in model_name_lower for term in ["line_item", "lineitem", "line_items", "lineitems"]):
                        columns = [
                            {"name": "line_item_key", "data_type": "integer", "expression": "", "description": "Primary line item identifier"},
                            {"name": "order_key", "data_type": "integer", "expression": "", "description": "Reference to order"},
                            {"name": "product_key", "data_type": "integer", "expression": "", "description": "Reference to product"},
                            ...
                            {"name": "discount", "data_type": "decimal", "expression": "", "description": "Item discount"}
                        ]
                    # Supplier related tables
                    elif any(term in model_name_lower for term in ["supplier", "suppliers", "vendor", "vendors"]):
                        columns = [
                            {"name": "supplier_key", "data_type": "integer", "expression": "", "description": "Primary supplier identifier"},
                            {"name": "name", "data_type": "string", "expression": "", "description": "Supplier name"},
                            {"name": "address", "data_type": "string", "expression": "", "description": "Supplier address"},
                            {"name": "phone", "data_type": "string", "expression": "", "description": "Supplier phone number"}
                        ]
                    # Part related tables
                    elif any(term in model_name_lower for term in ["part", "parts"]):
                        columns = [
                            {"name": "part_key", "data_type": "integer", "expression": "", "description": "Primary part identifier"},
                            {"name": "name", "data_type": "string", "expression": "", "description": "Part name"},
                            {"name": "manufacturer", "data_type": "string", "expression": "", "description": "Part manufacturer"},
                            {"name": "brand", "data_type": "string", "expression": "", "description": "Part brand"}
                        ]
                    # Location/Region related tables
                    elif any(term in model_name_lower for term in ["region", "regions", "location", "locations"]):
                        columns = [
                            {"name": "region_key", "data_type": "integer", "expression": "", "description": "Primary region identifier"},
                            {"name": "name", "data_type": "string", "expression": "", "description": "Region name"},
                            {"name": "country", "data_type": "string", "expression": "", "description": "Country name"}
                        ]
                    # Specifically for TPCH models
                    elif "tpch" in model_name_lower:
                        if "orders" in model_name_lower:
                            columns = [
                                {"name": "o_orderkey", "data_type": "integer", "expression": "", "description": "Primary order identifier"},
                                {"name": "o_custkey", "data_type": "integer", "expression": "", "description": "Reference to customer"},
                                {"name": "o_orderstatus", "data_type": "string", "expression": "", "description": "Order status"},
                                {"name": "o_totalprice", "data_type": "decimal", "expression": "", "description": "Total order price"},
                                {"name": "o_orderdate", "data_type": "date", "expression": "", "description": "Date order was placed"},
                                {"name": "o_clerk", "data_type": "string", "expression": "", "description": "Clerk who processed the order"}
                            ]
                        elif "lineitem" in model_name_lower:
                            columns = [
                                {"name": "l_orderkey", "data_type": "integer", "expression": "", "description": "Reference to order"},
                                {"name": "l_partkey", "data_type": "integer", "expression": "", "description": "Reference to part"},
                                {"name": "l_suppkey", "data_type": "integer", "expression": "", "description": "Reference to supplier"},
                                {"name": "l_linenumber", "data_type": "integer", "expression": "", "description": "Line item number within order"},
                                {"name": "l_quantity", "data_type": "decimal", "expression": "", "description": "Quantity ordered"},
                                {"name": "l_extendedprice", "data_type": "decimal", "expression": "", "description": "Extended price (quantity * price)"}
                            ]
                        elif "customer" in model_name_lower:
                            columns = [
                                {"name": "c_custkey", "data_type": "integer", "expression": "", "description": "Primary customer identifier"},
                                {"name": "c_name", "data_type": "string", "expression": "", "description": "Customer name"},
                                {"name": "c_address", "data_type": "string", "expression": "", "description": "Customer address"},
                                {"name": "c_nationkey", "data_type": "integer", "expression": "", "description": "Reference to nation"}
                            ]
                    # Generic columns for any other source/staging model
                    else:
                        # Extract base name (without stg_ or source_)
                        base_name = model_name_lower.replace("stg_", "").replace("source_", "")
                        
                        # Create sensible default columns based on table name
                        columns = [
                            {"name": f"{base_name}_key", "data_type": "integer", "expression": "", "description": f"Primary {base_name} identifier"},
                            {"name": "name", "data_type": "string", "expression": "", "description": f"{base_name} name"},
                            {"name": "description", "data_type": "string", "expression": "", "description": f"{base_name} description"},
                            {"name": "created_at", "data_type": "timestamp", "expression": "", "description": "Creation timestamp"},
                            {"name": "updated_at", "data_type": "timestamp", "expression": "", "description": "Last update timestamp"}
                        ]
                # Generate basic columns for fact tables
                elif "fct_" in model_name_lower:
                    # Extract the entity name from fct_entity
                    entity_name = model_name_lower.replace("fct_", "")
                    
                    # Generate typical fact table columns with measures and dimension keys
                    columns = [
                        {"name": f"{entity_name}_key", "data_type": "integer", "expression": "", "description": f"Primary {entity_name} identifier"},
                        {"name": "date_key", "data_type": "integer", "expression": "", "description": "Reference to date dimension"},
                        {"name": "customer_key", "data_type": "integer", "expression": "", "description": "Reference to customer dimension"},
                        {"name": "product_key", "data_type": "integer", "expression": "", "description": "Reference to product dimension"},
                        {"name": "location_key", "data_type": "integer", "expression": "", "description": "Reference to location dimension"},
                        {"name": "amount", "data_type": "decimal", "expression": "", "description": "Measure: Amount value"},
                        {"name": "quantity", "data_type": "integer", "expression": "", "description": "Measure: Quantity value"},
                        {"name": "count", "data_type": "integer", "expression": "", "description": "Measure: Count value"}
                    ]
                # Generate basic columns for dimension tables
                elif "dim_" in model_name_lower:
                    # Extract the entity name from dim_entity
                    entity_name = model_name_lower.replace("dim_", "")
                    
                    # Generate typical dimension table columns
                    columns = [
                        {"name": f"{entity_name}_key", "data_type": "integer", "expression": "", "description": f"Primary {entity_name} key"},
                        {"name": f"{entity_name}_id", "data_type": "integer", "expression": "", "description": f"Source system {entity_name} ID"},
                        {"name": "name", "data_type": "string", "expression": "", "description": f"{entity_name} name"},
                        {"name": "description", "data_type": "string", "expression": "", "description": f"{entity_name} description"},
                        {"name": "valid_from", "data_type": "timestamp", "expression": "", "description": "Validity start date"},
                        {"name": "valid_to", "data_type": "timestamp", "expression": "", "description": "Validity end date"}
                    ]
                # For any model that doesn't match specific patterns, create generic columns
                else:
                    # Create generic column names based on the model name
                    columns = [
                        {"name": f"{model_name.lower()}_id", "data_type": "integer", "expression": "", "description": f"Primary {model_name} identifier"},
            ...
            for column in columns:
                ...
                col_type = "regular"
                if col_name.lower().endswith('_key') or col_name.lower().endswith('_id') or col_name.lower().endswith('key') or col_name.lower().endswith('id'):
                    # Check if this looks like a primary or foreign key
                    model_name_root = model_name.lower().replace("stg_", "").replace("fct_", "").replace("dim_", "").replace("source_", "")
                    # If the column name contains the model name root, likely a primary key
                    if model_name_root in col_name.lower():
        ...
        # Add a new function to infer column relationships based on naming patterns
        def infer_column_relationships():
            """
            Infer column relationships between models based on naming conventions and patterns
            when explicit relationships couldn't be determined from SQL analysis.
            """
            logger.info("Inferring column relationships based on naming patterns")
            
            # Get all columns by model
            columns_by_model = {}
            for col in lineage_data["columns"]:
                model_id = col["modelId"]
                if model_id not in columns_by_model:
                    columns_by_model[model_id] = []
                columns_by_model[model_id].append(col)
            
            # Get all edges (model relationships)
            edges_by_source = {}
            for edge in lineage_data["edges"]:
                source = edge["source"]
                target = edge["target"]
            ...
            # Patterns to match for potential relationships
            key_patterns = [
                # Common key patterns
                r'^(\w+)_key$',
                r'^(\w+)_id$',
                ...
                r'^s_(\w+)key$',
                # Generic patterns
                r'^(\w+)_sk$',  # surrogate key
                r'^(\w+)_fk$',  # foreign key
                r'^(\w+)_pk$',  # primary key
                r'^fk_(\w+)$',  # foreign key alternative pattern
            ]
            
            # Track already created relationships to avoid duplicates
            created_relationships = set()
            
            # For each model that has downstream models
            for source_model, target_models in edges_by_source.items():
                source_columns = columns_by_model.get(source_model, [])
                
                # Skip if source model has no columns
                if not source_columns:
                    continue
                
                # For each target model
                for target_model in target_models:
                    target_columns = columns_by_model.get(target_model, [])
                    
                    # Skip if target model has no columns
                    if not target_columns:
                        continue
                    
                    # 1. First try to find exact column name matches (same name in both models)
                    for source_col in source_columns:
                        source_col_name = source_col["name"].lower()
                        for target_col in target_columns:
                            target_col_name = target_col["name"].lower()
                            
                            # If exact name match on a key-like column (likely a join key)
                            if (source_col_name == target_col_name and 
                                (source_col_name.endswith('_key') or 
                                 source_col_name.endswith('_id') or
                                 source_col_name.endswith('key') or 
                                 source_col_name.endswith('id') or
                                 'key' in source_col_name or
                                 'id' in source_col_name)):
                                
                                # Create a relationship
                                relationship_key = f"{source_col['id']}:{target_col['id']}"
                                if relationship_key not in created_relationships:
                                    lineage_data["column_lineage"].append({
                                        "source": source_col["id"],
                                        "target": target_col["id"]
                                    })
                                    created_relationships.add(relationship_key)
                                    logger.info(f"Inferred column relationship based on exact name match: {source_model}.{source_col_name} -> {target_model}.{target_col_name}")
                    
                    # 2. Try to match based on entity name patterns in keys
                    for source_col in source_columns:
                        source_col_name = source_col["name"].lower()
                        
                        # Skip if not a key column
                        if not (source_col_name.endswith('_key') or 
                                source_col_name.endswith('_id') or
                                source_col_name.endswith('key') or 
                                source_col_name.endswith('id')):
                            continue
                        
                        # Extract the entity name from the key
                        source_entity = None
                        for pattern in key_patterns:
                            match = re.match(pattern, source_col_name)
                            if match:
                        ...
                        # Find target columns with matching entity name
                        for target_col in target_columns:
                            target_col_name = target_col["name"].lower()
                            
                            # Try to extract entity from target column
                            for pattern in key_patterns:
                                match = re.match(pattern, target_col_name)
                                if match and match.group(1) == source_entity:
                                    # Create a relationship
                                    relationship_key = f"{source_col['id']}:{target_col['id']}"
                                    if relationship_key not in created_relationships:
                                        lineage_data["column_lineage"].append({
                                        ...
                                        created_relationships.add(relationship_key)
                                        logger.info(f"Inferred column relationship based on entity pattern: {source_model}.{source_col_name} -> {target_model}.{target_col_name}")
                                    break
                    
                    # 3. Match on common column patterns in data warehousing
                    common_keys = {
                        'customer': ['customer_key', 'customer_id', 'customerkey', 'customerid', 'cust_key', 'cust_id', 'c_custkey'],
                        'order': ['order_key', 'order_id', 'orderkey', 'orderid', 'o_orderkey'],
                        'product': ['product_key', 'product_id', 'productkey', 'productid', 'prod_key', 'prod_id', 'p_partkey'],
                        'supplier': ['supplier_key', 'supplier_id', 'supplierkey', 'supplierid', 'supp_key', 'supp_id', 's_suppkey', 'l_suppkey'],
                        'part': ['part_key', 'part_id', 'partkey', 'partid', 'p_partkey', 'l_partkey'],
                        'date': ['date_key', 'date_id', 'datekey', 'dateid', 'd_datekey'],
                        'region': ['region_key', 'region_id', 'regionkey', 'regionid', 'r_regionkey'],
                        'nation': ['nation_key', 'nation_id', 'nationkey', 'nationid', 'n_nationkey', 'c_nationkey'],
                        'time': ['time_key', 'time_id', 'timekey', 'timeid', 't_timekey'],
                    }
                    
                    # For each entity type (customer, order, etc)
                    for entity, key_variants in common_keys.items():
                        # Find source columns matching this entity
                        source_entity_cols = []
                        for source_col in source_columns:
                            if source_col["name"].lower() in key_variants:
                                source_entity_cols.append(source_col)
                        
                        # Find target columns matching this entity
                        target_entity_cols = []
                        for target_col in target_columns:
                            if target_col["name"].lower() in key_variants:
                                target_entity_cols.append(target_col)
                        
                        # Create relationships between matching entity columns
                        for source_col in source_entity_cols:
                            for target_col in target_entity_cols:
                                relationship_key = f"{source_col['id']}:{target_col['id']}"
                                if relationship_key not in created_relationships:
                                    lineage_data["column_lineage"].append({
                                        "source": source_col["id"],
                                    ...
                                    created_relationships.add(relationship_key)
                                    logger.info(f"Inferred column relationship based on common {entity} key pattern: {source_model}.{source_col['name']} -> {target_model}.{target_col['name']}")
            
            return len(created_relationships)
        
        # Call the function to infer column relationships
        if len(lineage_data["column_lineage"]) == 0:
            num_inferred = infer_column_relationships()
            logger.info(f"Inferred {num_inferred} column relationships based on naming patterns")
    ...
    # Add this helper function after _generate_key_changes
    def _check_column_added(self, original_code: str, enhanced_code: str, source_column_name: str, new_column_name: str, aggregation_type: str) -> bool:
        """Check if the required column was actually added to the enhanced code."""
        if not source_column_name or not new_column_name or not aggregation_type:
            return False
            
        # Standardize aggregation type for regex
        agg_pattern = "avg" if aggregation_type.lower() in ["avg", "average"] else aggregation_type.lower()
        
        # Check for various patterns of column addition
        patterns = [
            # Direct column calculation in select
            rf'{agg_pattern}\s*\(\s*{re.escape(source_column_name)}\s*\)\s+as\s+{re.escape(new_column_name)}',
            # Column reference after being defined in a CTE
            rf'{re.escape(new_column_name)}\s*(?:,|$)',
            # Column calculation in CTE definition
            rf'{agg_pattern}\s*\(\s*{re.escape(source_column_name)}\s*\)\s+as\s+{re.escape(new_column_name)}'
        ]
        
        # Check if new column exists in the enhanced code but not in the original
        for pattern in patterns:
            enhanced_match = re.search(pattern, enhanced_code, re.IGNORECASE)
            original_match = re.search(pattern, original_code, re.IGNORECASE)
            
            if enhanced_match and not original_match:
                return True
        
        return False

    # Update the _validate_code_enhancement_response method to check for the added column
    def _validate_code_enhancement_response(self, response: str, state: Dict[str, Any]) -> str:
        """Validate and format code enhancement responses."""
        # Extract original question for context
        original_question = ""
